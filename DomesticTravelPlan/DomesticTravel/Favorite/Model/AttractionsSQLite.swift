...
struct AttractionsSQLite {
    
    private var db: Connection!
    private let collection = Table("Attractions") //表名
    ...
    private let toldeScribe = Expression<String>("toldeScribe") //介绍
    private let openTime = Expression<String>("openTime") //营业时间
    private let ticketInfo = Expression<String>("Ticketinfo") //停车位
    ...
    //插入数据
    func insertData(_data: AttractionsInfo) {
        do {
            if searchCollect(_id: _data.id) == 0 && !_data.id.elementsEqual("") {
                let insert = collection.insert(image1 <- _data.image1, image2 <- _data.image2, image3 <- _data.image3, name <- _data.name, tel <- _data.tel, toldeScribe <- _data.toldeScribe, openTime <- _data.openTime, ticketInfo <- _data.ticketInfo, website <- _data.website, add <- _data.add, px <- _data.px, py <- _data.py, mId <- _data.id)
                try db.run(insert)
            }
        } catch {
    ...
    /// 更新数据
    func upDate(_data: AttractionsInfo) {
        do {
            if searchCollect(_id: _data.id) != 0 {
                let currUser = collection.filter(mId == _data.id)
                let insertVideoID = currUser.update(image1 <- _data.image1, image2 <- _data.image2, image3 <- _data.image3, name <- _data.name, tel <- _data.tel, toldeScribe <- _data.toldeScribe, openTime <- _data.openTime, ticketInfo <- _data.ticketInfo, website <- _data.website, add <- _data.add, px <- _data.px, py <- _data.py, mId <- _data.id)
    ...
    //读取数据
    func readData() -> [AttractionsInfo] {
        var collectionArray: [AttractionsInfo] = []
        ...
        for user in try! db.prepare(collection) {
            ...
            pars["Tel"] = user[tel]
            pars["Add"] = user[add]
            pars["Toldescribe"] = user[toldeScribe]
            pars["Px"] = user[px]
            pars["Py"] = user[py]
            ...
            pars["Ticketinfo"] = user[ticketInfo]
            pars["Website"] = user[website]
            let collectionData: AttractionsInfo = AttractionsInfo(JSON: pars)!
