...
struct HotelSQLite {
    ...
    private let collection = Table("Hotel") //表名
    ...
    private let serviceInfo = Expression<String>("serviceInfo") //营业时间
    ...
    private let money = Expression<String>("money") //他的$$
    ...
    //插入数据
    func insertData(_data: HotelInfo) {
        do {
            if searchCollect(_id: _data.id) == 0 && !_data.id.elementsEqual("") {
                let insert = collection.insert(image1 <- _data.image1, image2 <- _data.image2, image3 <- _data.image3, name <- _data.name, tel <- _data.tel, description <- _data.description, serviceInfo <- _data.serviceInfo, parkingInfo <- _data.parkingInfo, website <- _data.website, add <- _data.add, px <- _data.px, py <- _data.py, mId <- _data.id, money <- _data.money)
                try db.run(insert)
            }
        } catch {
    ...
    /// 更新数据
    func upDate(_data: HotelInfo) {
        do {
            if searchCollect(_id: _data.id) != 0 {
                let currUser = collection.filter(mId == _data.id)
                let insertVideoID = currUser.update(image1 <- _data.image1, image2 <- _data.image2, image3 <- _data.image3, name <- _data.name, tel <- _data.tel, description <- _data.description, serviceInfo <- _data.serviceInfo, parkingInfo <- _data.parkingInfo, website <- _data.website, add <- _data.add, px <- _data.px, py <- _data.py, mId <- _data.id, money <- _data.money)
    ...
    //读取数据
    func readData() -> [HotelInfo] {
        var collectionArray: [HotelInfo] = []
        ...
        for user in try! db.prepare(collection) {
            ...
            pars["Serviceinfo"] = user[serviceInfo]
            pars["Parkinginfo"] = user[parkingInfo]
            pars["Website"] = user[website]
            pars["Spec"] = user[money]
            let collectionData: HotelInfo = HotelInfo(JSON: pars)!
